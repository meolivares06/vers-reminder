...
class ImageCacheService {
  ...
  /// Maximum number of nature images held in cache.
  ///
  /// Ten images provide enough variety for the current wallpaper rotation
  /// scale (~7 frequencies × 3 categories) without exhausting device storage.
  /// This limit is intentionally small and should only be raised when the
  /// asset library grows significantly beyond its current size.
  ...
  Future<void> _repairCache() async {
    ...
    for (final file in files) {
      // Read only the first 2 bytes (JPG magic number 0xFF 0xD8) instead of
      // the whole file. openRead + expand + take(2) avoids loading large
      // images into memory just for a signature check.
      final bytes = await file.openRead().expand((c) => c).take(2).toList();
