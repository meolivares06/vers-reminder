...
void main() {
  group('WallpaperGenerator screen-size output', () {
    ...
    test('native dimensions preserved when screen params omitted', () {
      ...
      expect(srcImage!.width, 100,
          reason: 'without resize, width should match source');
      expect(srcImage.height, 200,
    ...
    // ── Task 3.3 ──
    test('fallback to 1080×1920 when SharedPreferences returns null',
        () async {
      ...
      expect(screenWidth, 1080,
          reason: 'null screen_width should fall back to 1080');
      expect(screenHeight, 1920,
          reason: 'null screen_height should fall back to 1920');
    });

    // ── Task 3.4 ──
    test('SharedPreferences cached dimensions are read and forwarded',
        () async {
    ...
    test('corrupt image bytes cause decodeImage to return null', () {
      ...
      final corruptPath =
          '${Directory.systemTemp.path}/corrupt_test_image.bin';
    ...
    test('generateAndSetWallpaper returns error when image is missing',
        () async {
    ...
    test('main.dart write-once guard does not overwrite existing dimensions',
      ...
      expect(prefs.getInt('screen_width'), 720,
          reason: 'write-once guard should keep original 720, not 1080');
      expect(prefs.getInt('screen_height'), 1280,
          reason: 'write-once guard should keep original 1280, not 1920');
      expect(prefs.getInt('screen_dim_version'), 2,
          reason: 'version should remain unchanged');
    });

    test('version mismatch clears stale dimensions for re-cache',
        () async {
      ...
      expect(prefs.containsKey('screen_width'), false,
          reason: 'stale screen_width should be removed');
      expect(prefs.containsKey('screen_height'), false,
  ...
  group('_composite() pipeline', () {
    ...
    test('compositeFromBytes returns PNG bytes matching specified dimensions',
        () async {
      ...
      expect(decoded!.width, 1080,
          reason: 'width should match screenWidth');
      expect(decoded.height, 2340,
    ...
    test('compositeFromBytes returns null for corrupt input bytes', () async {
      ...
      expect(result, isNull,
          reason: 'corrupt bytes should return null');
    });
    ...
    test(
        'compositing with calibratedInset crops before resize',
        () async {
