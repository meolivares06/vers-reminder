...
void main() {
  ...
  testWidgets(
    ...
    expect(find.text('Update available: v1.2.0+1'), findsAtLeastNWidgets(2),
        reason: 'tile subtitle + confirm dialog title');
    expect(find.byType(AlertDialog), findsOneWidget);
    expect(
      find.text('A new version (v1.2.0+1) is available. Download '
  ...
  testWidgets(
      ...
      'the stream advances and can be canceled', (tester) async {
    ...
    expect(find.text('Downloading update... 0%'), findsOneWidget,
  ...
  testWidgets('Sc.3 download completes: progress closes, Install action shown',
      (tester) async {
    ...
    expect(find.text('Download complete. Install the update now?'),
        findsOneWidget);
  ...
  testWidgets(
    ...
    expect(find.byType(LinearProgressIndicator), findsNothing);
    expect(find.text('Download complete. Install the update now?'), findsOneWidget,
  ...
  testWidgets('Sc.4 installing fires the install flow with the downloaded APK',
      (tester) async {
    final service = _FakeUpdateService(
      checkResult: _availableResult,
      installResult: true,
    ...
    expect(service.installCalls, ['/fake/downloaded/vers-reminder-arm64.apk'],
        reason: 'Install triggers the service install flow with the APK');
    expect(find.byType(AlertDialog), findsNothing);
    expect(find.text("Couldn't install the update"), findsNothing,
        reason: 'no failure feedback on a successful install');
  });

  testWidgets('Sc.4b install hands back the release page URL (not the APK URL)',
      (tester) async {
    final service = _FakeUpdateService(
      checkResult: _availableResult,
      installResult: false,
    ...
    expect(service.installReleaseUrls, [
      'https://github.com/meolivares06/vers-reminder/releases/tag/v1.2.0+1',
    ], reason: 'the browser fallback must land on the release page, not the '
        'raw APK download URL');
    expect(find.text("Couldn't install the update"), findsOneWidget,
  ...
  testWidgets(
    ...
    expect(find.byType(AlertDialog), findsNothing);
    expect(find.text('Update available: v1.2.0+1'), findsNothing,
        reason: 'cancelling the dialog clears the "available" subtitle and '
  ...
  testWidgets(
      ...
      'the tile re-triggerable', (tester) async {
    final service = _FakeUpdateService(
      checkResult: _availableResult,
    ...
    await _pumpAndPause(tester);
    await _pumpAndPause(tester);
    expect(service.downloadCalls, hasLength(2),
  ...
  testWidgets('Change now renders via the shared loader button',
    ...
    expect(find.byType(AsyncActionButton), findsWidgets,
