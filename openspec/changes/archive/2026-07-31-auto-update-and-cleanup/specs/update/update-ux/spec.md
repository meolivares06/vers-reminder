...
**PR 2 — auto-update.** The system MUST add a "Check for updates" `ListTile` to the About section of the settings screen. When an update is available, the section MUST show the subtitle "Update available: vX.Y.Z+N" and an action/badge. The system MUST NOT auto-prompt on app start — the update check may run silently to surface a subtle indicator, but MUST NOT show an intrusive modal automatically.
