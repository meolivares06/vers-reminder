# Delta for cleanup-temp

## ADDED Requirements

### Requirement: Temp wallpaper cleanup

**PR 1 — cleanup.** The system MUST maintain a `TempCleanupService` that, on app start, sweeps `getTemporaryDirectory()` for files matching `wallpaper_*.png` and deletes every one EXCEPT the file whose absolute path equals `last_wallpaper_path` in SharedPreferences.
