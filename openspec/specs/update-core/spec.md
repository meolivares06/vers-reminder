...
Self-update capability for the app distributed via GitHub Releases: checks the latest release, compares versions numerically, and downloads the arm64 APK with progress reporting.

## Requirements

### Requirement: UpdateService version check

The system MUST provide an `UpdateService` that checks for a newer release by calling `https://api.github.com/repos/meolivares06/vers-reminder/releases/latest`.
...
When an update is confirmed, the system MUST download the arm64 APK asset to `{updatesDir}/{safeFileName}` using the `browser_download_url` from the release JSON. The system MUST use the URL exactly as provided — already URL-encoded (e.g. `%2B` for `+`) — and MUST NOT re-encode it.
