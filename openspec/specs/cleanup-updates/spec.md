...
Manages a dedicated download directory for APK files so downloaded updates do not accumulate on disk and stale or partial downloads are removed before new ones begin.

## Requirements

### Requirement: Updates dir management

The system MUST maintain an `UpdateCleanupService` that manages a dedicated download directory at `{getApplicationSupportDirectory()}/updates/` for downloaded APK files. The service MUST be reusable by the auto-update flow.
...
The system MUST NOT perform any version checking, network access, or install logic as part of updates dir management.
